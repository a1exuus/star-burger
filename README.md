...
YANDEX_API_KEY=ваш ключ геокодера яндекс.
```
Ключ геокодера Яндекс можно получить по [ссылке](https://developer.tech.yandex.ru/services). Нужно зарегистрироваться или войти в аккаунт на сайте и нажать **Подключить API**. Дальше выбрать **API Геокодера**, после чего следовать инструкциям.
